...
const Dashboard = () => {
  ...
  const totalChanges = added.map(
    (val, i) =>
      val + movedOut[i] + deceased[i] + changeOwner[i] + splitHousehold[i]
  );
  ...
  const residentChangesOptions = {
    ...
    scales: {
      ...
      y: {
        stacked: true,
        beginAtZero: true,
        title: { display: true, text: "Số nhân khẩu" },
        ticks: { stepSize: 1 },
      },
    },
    ...
    datasets: [
      {
        ...
        hoverBackgroundColor: ["#1D4ED8", "#60A5FA"],
        borderWidth: 0,
      },
    ],
  };
  ...
  const genderData = {
    ...
    datasets: [
      {
        data: [65, 35],
        backgroundColor: ["#7d9cc9ff", "#ee5555ff"],
        hoverBackgroundColor: ["#64748B", "#EF4444"],
        borderWidth: 0,
      },
    ],
  };
