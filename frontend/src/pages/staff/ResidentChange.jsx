...
export default function ResidentChangeCreate() {
  ...
  const [residentHouseholdId, setResidentHouseholdId] = useState(null)
  const [residentIsOwner, setResidentIsOwner] = useState(false)

  const needsNewOwner = useMemo(() => {
    if (![4, 7].includes(ctNum)) return false
    if (!residentResult?.id) return false
    return !!residentIsOwner
  }, [ctNum, residentResult?.id, residentIsOwner])
  ...
  useEffect(() => {
    ...
    setResidentHouseholdId(null)
    setResidentIsOwner(false)
  ...
  useEffect(() => {
    if (!needsNewOwner) return
    if (!residentHouseholdId) return

    axios
      .get(`${API_BASE}/households/${residentHouseholdId}/members`, { headers: authHeaders() })
      .then(res => {
        const list = res.data?.data || []
        setHouseholdMembers(list)
        setNewOwnerId(null)
  ...
  }, [needsNewOwner, residentHouseholdId])
  ...
  const ownerCandidates = useMemo(() => {
    if (!needsNewOwner) return []
    const rid = Number(residentResult?.id)
    return (householdMembers || []).filter(m => Number(m.id) !== rid)
  }, [needsNewOwner, householdMembers, residentResult?.id])
  ...
  async function handleCreateSubmit(e) {
    ...
    if (needsNewOwner) {
      if (!residentHouseholdId) return alert("Không xác định được hộ khẩu của chủ hộ")
      if (ownerCandidates.length < 1) return alert("Hộ khẩu không còn thành viên khác để chọn làm chủ hộ mới")
      if (!newOwnerId) return alert("Chủ hộ chuyển đi/khai tử: Vui lòng chọn chủ hộ mới")
    }
          ...
          <div className="rc-detail-grid">
            ...
            {isUseResident && (
              <div className="rc-detail-item rc-wide rc-suggest-wrap">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          if ([2, 4, 7].includes(ctNum)) {
                            ...
                            const rel =
                              detail?.relationToOwner ||
                              detail?.resident?.relationToOwner ||
                              detail?.resident?.relation ||
                              ""

                            const isOwner = String(rel || "").trim() === "Chủ hộ"
                            setResidentIsOwner(isOwner)

                            const hh =
                              detail?.household ||
                              detail?.resident?.household ||
                              detail?.resident?.householdInfo ||
                              null

                            const hid = Number(hh?.id || detail?.householdId || detail?.resident?.householdId || 0)
                            setResidentHouseholdId(hid || null)

                            if (hh?.id) {
                              setSelectedHousehold(hh)
                              setHouseholdSearch(hh.householdCode || "")
                            }
                ...
                {needsNewOwner && (
                  <div className="rc-sub-text" style={{ marginTop: 6, color: "#b45309" }}>
                    Chủ hộ {ctNum === 4 ? "chuyển đi" : "khai tử"}: bắt buộc chọn chủ hộ mới.
                  </div>
                )}
              </div>
            )}
            ...
            {needsNewOwner && (
              <div className="rc-detail-item rc-wide">
                <div className="rc-detail-label">Chọn chủ hộ mới</div>

                {ownerCandidates.length === 0 ? (
                  <div className="rc-sub-text">Không có thành viên khác để chọn</div>
                ) : (
