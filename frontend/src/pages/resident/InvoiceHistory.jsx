...
export default function FeeHistory() {
  ...
  const filteredFees = useMemo(() => {
    return feeRecords
      .filter((f) => (f.feeType?.name ?? "").toLowerCase().includes(search.toLowerCase()))
      .filter((f) => {
  ...
  const pagedFees = filteredFees.slice((currentPage - 1) * pageSize, currentPage * pageSize)
  ...
  const downloadPdf = async (feeRecordId) => {
    try {
      const token = localStorage.getItem("token")
      const res = await axios.get(`http://localhost:5000/api/household/fees/invoice/${feeRecordId}/pdf`, {
        headers: { Authorization: `Bearer ${token}` },
        responseType: "blob",
        validateStatus: () => true
      })
      ...
      a.href = url
      a.download = `invoice_${feeRecordId}.pdf`
      ...
      alert("Không tải được hóa đơn PDF.")
    }
  }
