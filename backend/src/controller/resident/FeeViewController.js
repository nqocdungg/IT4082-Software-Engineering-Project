...
export const getPendingFees = async (req, res) => {
  try {
    ...
    const mandatoryTypesRaw = await prisma.feeType.findMany({
      where: {
        isMandatory: true,
        isActive: true
      },
    ...
    const mandatoryTypes = mandatoryTypesRaw.filter((t) => !isExpiredFeeType(t))
    ...
    const contributionTypes = contributionTypesRaw.filter((t) => !isExpiredFeeType(t))
 
    const contributionFees = contributionTypes.map((type) => {
...
export const processPayment = async (req, res) => {
  ...
  try {
    ...
    if (type === "MANDATORY") {
      ...
      const typesRaw = await prisma.feeType.findMany({
        where: {
          id: { in: ids },
          isMandatory: true,
          isActive: true
      ...
      const types = typesRaw.filter((t) => !isExpiredFeeType(t))
    ...
    if (type === "CONTRIBUTION") {
      ...
      const validTypesRaw = await prisma.feeType.findMany({
        where: {
          id: { in: typeIds },
          isMandatory: false,
          isActive: true
      ...
      const validTypes = validTypesRaw.filter((t) => !isExpiredFeeType(t))
