...
export const getMyHouseholdFees = async (req, res) => {
  try {
    ...
    if (!user || !user.householdId) {
      return res.status(400).json({ message: "Household ID could not be determined." })
    }
    ...
    console.error("Error in getMyHouseholdFees:", error)
    res.status(500).json({ message: "Internal server error while fetching fee records." })
  }
}
