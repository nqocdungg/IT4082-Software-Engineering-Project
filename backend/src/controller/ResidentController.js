...
export const getResidents = async (req, res) => {
  try {
    ...
    if (gender && gender !== "ALL") where.gender = gender === "Nam" ? "M" : "F"
    if (search) {
      where.OR = [
    ...
    const residents = await prisma.resident.findMany({
      ...
      include: { household: { select: { id: true, address: true } } }
    })
...
export const createResident = async (req, res) => {
  try {
    const {
      residentCCCD, fullname, dob, gender,
      relationToOwner, status, householdId
    } = req.body
    ...
    return res.status(201).json({ message: "Resident created", data: newResident })
  } catch (err) {
...
export const updateResident = async (req, res) => {
  try {
    ...
    return res.json({ message: "Resident updated", data: updated })
  } catch (err) {
